...
{
	WavesTextured::WavesTextured(DX::DeviceResources* d3dres) : 
		ISandbox(d3dres),
		m_Effects(d3dres->GetD3DDevice(), GetDefaultTexture(d3dres), GetDefaultWolrdConstants())
	{
		...
		m_LandGeometry.CreateShaders(GetDeviceResources()->GetD3DDevice(), L"resources/box_texture/box_vs.cso", L"resources/box_texture/box_ps.cso");
		...
		m_WavesGeometry.CreateShaders(GetDeviceResources()->GetD3DDevice(), L"resources/box_texture/box_vs.cso", L"resources/box_texture/box_ps.cso");
	...
	void WavesTextured::OnFrame(uint64_t ticks)
	{
		...
		for (auto [geometry, effect_info, texture] : {
			std::tuple(&m_LandGeometry, &m_Land, m_LandTexture.Get()),
			std::tuple(&m_WavesGeometry, &m_Waves, m_WavesTexture.Get())
			})
		{
	...
	void WavesTextured::OnImGuiDraw()
	{
	...
	void WavesTextured::BuildLandAndWavesMesh()
	{
	...
	void WavesTextured::UpdateWavesMesh(uint64_t ticks)
	{
	...
	auto WavesTextured::GetDefaultWolrdConstants() ->
	...
	auto WavesTextured::GetDefaultTexture(DX::DeviceResources* d3dres) ->
