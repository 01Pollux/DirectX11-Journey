...
namespace Pleiades::Sandbox::TextureBoxMutliple
{
	class EffectManager
	{
	public:
		...
		{
			DX::ComPtr<ID3D11ShaderResourceView> Texture[2];
		...
		{
			m_NBuffer.Texture[index] = texture;
		}
		...
		void Bind(ID3D11DeviceContext* d3dcontext)
		{
			...
			ID3D11ShaderResourceView* textures[]{ m_NBuffer.Texture[0].Get(), m_NBuffer.Texture[1].Get() };
