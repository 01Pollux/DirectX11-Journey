...
namespace Pleiades::Sandbox::AnimTextureBox
{
	class EffectManager
	{
	public:
		...
		{
			std::vector<DX::ComPtr<ID3D11ShaderResourceView>> Texture;
			size_t TexturePos{};
		...
		void Bind(ID3D11DeviceContext* d3dcontext)
		{
			...
			d3dcontext->PSSetShaderResources(
				0, 1, m_NBuffer.Texture[m_NBuffer.TexturePos].GetAddressOf()
			);
		...
		void NextFrame()
		{
			m_NBuffer.TexturePos = (m_NBuffer.TexturePos + 1) % m_NBuffer.Texture.size();
		}
