...
namespace DX
{
	...
	public:
		...
		static constexpr bool		  c_MSAAOn		 = false;
		static constexpr unsigned int c_MSAASampleCount = 0x4;
		static constexpr unsigned int c_MSAASampleQual  = 0x0;
		...
		void RegisterDeviceNotify(IDeviceNotify* deviceNotify) noexcept { m_deviceNotify.push_back(deviceNotify); }
		void UnregisterDeviceNotify(IDeviceNotify* deviceNotify) noexcept { m_deviceNotify.erase(std::find(m_deviceNotify.begin(), m_deviceNotify.end(), deviceNotify)); }
		void Present();
	...
	private:
		...
		// Collection of the IDeviceNotify listeners
		std::vector<IDeviceNotify*>                     m_deviceNotify;
	};
}
