...
)
{
    ...
    if (diffuse_factor > 0.f)
    {
        ...
        float spot_light = pow(max(dot(-to_light, light.Direction), 0.f), light.ExpFactor);
        float attenuation = spot_light / dot(light.Attenuation, float3(1.f, distance, distance * distance));
        
        res.Ambient *= spot_light;
        res.Specular = spec_factor * light.Specular * mat.Specular * attenuation;
        res.Diffuse = diffuse_factor * light.Diffuse * mat.Diffuse * attenuation;
    }
}
